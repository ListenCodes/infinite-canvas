...
+ [新增] 配置中心新增 Grok2API 与 Sub2API 渠道预设，适配 JSON 图片编辑、可恢复的异步视频任务及认证媒体下载。
+ [新增] 渠道内每个模型支持自动识别或手动选择协议默认、Grok2API、Sub2API 与自定义脚本适配器。
+ [修复] 修复画布图片请求被替换、取消或本地保存卡住后仍永久显示生成中的问题。
+ [修复] 多图生成改为逐张排队，避免慢速上游在同账号并发时超时；同时明确区分网关超时与模型脚本错误。
