...
export default function AdminPage() {
    ...
    const openModal = (next: Exclude<AdminModal, null>) => {
        ...
        if (next.kind === "model") {
            const capability = next.channel.capabilities[0];
            const policy = next.channel.capacityPolicies.find((item) => item.capability === capability);
            form.setFieldsValue({ model: "", capability, adapterType: next.channel.type, adapterVersion: 1, concurrencyLimit: policy?.concurrencyLimit ?? 3, rateLimitPerMinute: policy?.rateLimitPerMinute ?? 60, providerIdempotencySupported: false, creditAmount: "1", limits: "{}" });
        }
    ...
    const submitModal = async () => {
        ...
        try {
            if (modal.kind === "wallet") {
                ...
                await createAdminModel(modal.channel.id, { ...values, adapterType: modal.channel.type, adapterVersion: Number(values.adapterVersion), concurrencyLimit: Number(values.concurrencyLimit), rateLimitPerMinute: Number(values.rateLimitPerMinute), creditAmount: String(values.creditAmount), limits }, modalRequestKey);
            } else {
    ...
    <Modal open={Boolean(modal)} title={modal ? t(`admin.modals.${modal.kind}`) : ""} confirmLoading={saving} onCancel={() => { setModal(null); setModalRequestKey(""); form.resetFields(); }} onOk={() => void submitModal()} destroyOnHidden>
        <Form form={form} layout="vertical" className="pt-3" onValuesChange={(changed) => {
            if (modal?.kind !== "model" || typeof changed.capability !== "string") return;
            const policy = modal.channel.capacityPolicies.find((item) => item.capability === changed.capability);
            form.setFieldsValue({ concurrencyLimit: policy?.concurrencyLimit ?? 3, rateLimitPerMinute: policy?.rateLimitPerMinute ?? 60 });
        }}>
            ...
            {modal?.kind === "model" ? <><Form.Item name="model" label={t("admin.model")} rules={[{ required: true }]}><Input /></Form.Item><Form.Item name="capability" label={t("admin.capabilities")} rules={[{ required: true }]}><Select options={modal.channel.capabilities.map((value) => ({ value, label: value }))} /></Form.Item><Form.Item name="creditAmount" label={t("admin.price")} rules={[{ required: true, pattern: /^\d+$/ }]}><Input /></Form.Item><Form.Item name="concurrencyLimit" label={t("admin.channelConcurrency")} rules={[{ required: true, type: "integer", min: 1, max: 100 }]}><InputNumber min={1} max={100} className="w-full" /></Form.Item><Form.Item name="rateLimitPerMinute" label={t("admin.channelRatePerMinute")} rules={[{ required: true, type: "integer", min: 1, max: 100000 }]}><InputNumber min={1} max={100000} className="w-full" /></Form.Item><Form.Item name="providerIdempotencySupported" valuePropName="checked"><Checkbox>{t("admin.providerIdempotency")}</Checkbox></Form.Item><Form.Item name="limits" label={t("admin.limitsJson")}><Input.TextArea rows={4} /></Form.Item></> : null}
