...
for (let index = 0; index < argumentsList.length; index += 1) {
  ...
  if (value === "--allow-zero-drain-owners") {
    ...
    (value === "--env-file" ? envFileArguments : releaseManifestArguments).push(
      target,
    );
    index += 1;
...
const checkedFiles = [
  ...
  "infra/compose/oss/smoke.override.yaml",
...
const digestReference =
  /^[^\s:@]+(?:\/[^\s:@]+)*(?::[^\s@]+)?@sha256:[a-f0-9]{64}$/i;

function assertImmutableImage(value, source) {
  if (!digestReference.test(value))
    errors.push(`${source}: image must use an immutable sha256 digest`);
}
...
for (const line of lockContent
  ...
  if (!digestReference.test(value))
    errors.push(
      `infra/release/images.lock: invalid entry ${line.split(":", 1)[0]}`,
    );
...
for (const relativePath of checkedFiles) {
  ...
  if (/(?:image:|FROM)\s+[^\s]+:latest\b/i.test(content))
    errors.push(`${relativePath}: rolling latest image is forbidden`);
  for (const match of content.matchAll(/sha256:([a-f0-9]+)/gi)) {
    if (match[1].length !== 64)
      errors.push(`${relativePath}: invalid sha256 digest length`);
  }
  for (const match of content.matchAll(
    /^\s*(?:FROM|image:)\s+([^\s#]+).*$/gim,
  )) {
    const image = match[1];
    if (image.startsWith("${")) continue;
    if (
      relativePath === "infra/compose/local/compose.yaml" &&
      /^infinite-canvas-(?:web|api|worker):local$/.test(image)
    )
      continue;
    assertImmutableImage(image, relativePath);
    if (!lockedImages.has(image))
      errors.push(
        `${relativePath}: image is not recorded in infra/release/images.lock`,
      );
  }
}
...
const finalWebUser = /^USER\s+([^\s#]+)/im
  .exec(finalWebStage)?.[1]
  ?.toLowerCase();
...
for (const [relativePath, endpoint] of [
  ...
  if (!/^HEALTHCHECK\s+/im.test(finalStage) || !finalStage.includes(endpoint)) {
    errors.push(
      `${relativePath}: final stage must retain the production health probe ${endpoint}`,
    );
  }
}

const ossCompose = await readFile(
  resolve(repository, "infra/compose/oss/compose.yaml"),
  "utf8",
);
for (const required of [
  ...
  if (!ossCompose.includes(required))
    errors.push(
      `infra/compose/oss/compose.yaml: missing readiness invariant ${required}`,
    );
}

function composeService(content, service) {
  const match = new RegExp(
    `^  ${service}:\\r?\\n([\\s\\S]*?)(?=^  [a-z0-9-]+:|^volumes:|^networks:|(?![\\s\\S]))`,
    "m",
  ).exec(content);
  return match?.[1] ?? "";
}
  ...
  for (const [role, service] of [
    ...
    for (const required of [
      ...
      if (!service.includes(required))
        errors.push(`${overridePath}: ${role} Worker is missing ${required}`);
    }
  }
  for (const required of [
    "read_only: true",
    ...
    if (!oldBase.includes(required))
      errors.push(`${basePath}: old Worker is missing ${required}`);
    if (!candidate.includes(required))
      errors.push(`${overridePath}: new Worker is missing ${required}`);
  }
  if (/^\s+ports:/m.test(candidate))
    errors.push(`${overridePath}: worker-new must not expose host ports`);
}
...
for (const entry of await readdir(workflowsDirectory, {
  ...
  const content = await readFile(
    resolve(workflowsDirectory, entry.name),
    "utf8",
  );
  for (const match of content.matchAll(/^\s*uses:\s*([^\s#]+).*$/gim)) {
    ...
    if (!/@[a-f0-9]{40}$/.test(reference)) {
      errors.push(
        `${relativePath}: external action must use a full commit SHA (${reference})`,
      );
    }
  }
}
...
for (const topology of ["local", "cloud", "oss"]) {
  ...
  const workerEnvironment =
    topology === "cloud"
      ...
      : worker;
  if (
    !migration.includes("BUSINESS_DATABASE_MIGRATION_URL:") ||
    migration.includes("BUSINESS_DATABASE_PROVISION_URL:")
  ) {
    errors.push(
      `${relativePath}: database-migrate must receive only the migration-owner URL`,
    );
  }
  for (const required of [
    "BUSINESS_DATABASE_PROVISION_URL:",
    "BUSINESS_DATABASE_OBJECT_OWNER_ROLE:",
  ]) {
    if (!provision.includes(required))
      errors.push(
        `${relativePath}: database-provision-roles is missing ${required.slice(0, -1)}`,
      );
  }
  if (provision.includes("BUSINESS_DATABASE_MIGRATION_URL:")) {
    errors.push(
      `${relativePath}: database-provision-roles must not receive the migration-owner URL`,
    );
  }
  if (
    !workerEnvironment.includes("BUSINESS_DATABASE_URL_WORKER") ||
    workerEnvironment.includes("BUSINESS_DATABASE_URL_RECOVERY_AUDIT")
  ) {
    errors.push(
      `${relativePath}: worker must receive only the Worker runtime database URL`,
    );
  }
  if (
    !recoveryAudit.includes("BUSINESS_DATABASE_URL_RECOVERY_AUDIT") ||
    recoveryAudit.includes("BUSINESS_DATABASE_URL_WORKER")
  ) {
    errors.push(
      `${relativePath}: recovery-audit must receive only the read-only audit database URL`,
    );
  }
  if (!provision.includes("BUSINESS_DATABASE_RECOVERY_AUDIT_PASSWORD:")) {
    errors.push(
      `${relativePath}: database-provision-roles is missing BUSINESS_DATABASE_RECOVERY_AUDIT_PASSWORD`,
    );
  }
}

const recoveryCompose = await readFile(
  resolve(repository, "infra/compose/recovery/compose.yaml"),
  "utf8",
);
for (const forbidden of [
  ...
  if (recoveryCompose.includes(forbidden))
    errors.push(
      `infra/compose/recovery/compose.yaml: forbidden recovery boundary ${forbidden}`,
    );
}
...
const ossSmoke = parse(
  await readFile(resolve(repository, ossSmokePath), "utf8"),
);
const ossSmokeService = ossSmoke?.services?.["hatchet-terminal-smoke"];
if (!ossSmokeService) {
  errors.push(`${ossSmokePath}: hatchet-terminal-smoke service is required`);
} else {
  if (
    !Array.isArray(ossSmokeService.profiles) ||
    !ossSmokeService.profiles.includes("smoke")
  ) {
    errors.push(`${ossSmokePath}: terminal probe must use the smoke profile`);
  }
  if (
    JSON.stringify(ossSmokeService.networks) !==
    JSON.stringify(["hatchet-smoke"])
  ) {
    errors.push(
      `${ossSmokePath}: terminal probe must use only the isolated Hatchet smoke network`,
    );
  }
  for (const forbidden of [
    "BUSINESS_DATABASE",
    ...
    if (JSON.stringify(ossSmokeService).includes(forbidden))
      errors.push(
        `${ossSmokePath}: terminal probe exposes forbidden capability ${forbidden}`,
      );
  }
  if (
    ossSmokeService.read_only !== true ||
    !ossSmokeService.cap_drop?.includes("ALL") ||
    !ossSmokeService.security_opt?.includes("no-new-privileges:true")
  ) {
    errors.push(
      `${ossSmokePath}: terminal probe must retain the hardened container boundary`,
    );
  }
}
if (ossSmoke?.networks?.["hatchet-smoke"]?.internal !== true) {
  errors.push(`${ossSmokePath}: Hatchet smoke network must be internal`);
}

const serviceImageWorkflow = await readFile(
...
for (const required of [
  ...
  if (!serviceImageWorkflow.includes(required))
    errors.push(
      `.github/workflows/docker-image.yml: missing release manifest invariant ${required}`,
    );
}
...
function validateDrainEnvironment(
  environment,
  source,
  allowPlaceholder,
  allowZeroOwners = false,
) {
  for (const key of ["WORKER_OLD_IMAGE", "WORKER_NEW_IMAGE"]) {
    const value = environment[key];
    if (!value) errors.push(`${source}: ${key} is required`);
    else if (!(allowPlaceholder && /@sha256:replace-me$/.test(value)))
      assertImmutableImage(value, `${source}: ${key}`);
  }
  const ownerCounts = {};
  for (const subsystem of ["DISPATCHER", "RECONCILER"]) {
    ...
    if (
      ![oldValue, newValue].every(
        (value) => value === "true" || value === "false",
      )
    ) {
      errors.push(
        `${source}: ${subsystem.toLowerCase()} owner flags must be explicit booleans`,
      );
      continue;
    }
    ownerCounts[subsystem] =
      Number(oldValue === "true") + Number(newValue === "true");
  }
  if (allowZeroOwners) {
    if (ownerCounts.DISPATCHER !== 0 || ownerCounts.RECONCILER !== 0) {
      errors.push(
        `${source}: zero-owner handoff requires all dispatcher and reconciler flags to be false`,
      );
    }
    if (environment.GENERATION_WRITES_ENABLED !== "false") {
      errors.push(
        `${source}: zero-owner handoff requires GENERATION_WRITES_ENABLED=false`,
      );
    }
  } else {
    for (const subsystem of ["DISPATCHER", "RECONCILER"]) {
      if (ownerCounts[subsystem] !== 1) {
        errors.push(
          `${source}: exactly one ${subsystem.toLowerCase()} owner must be enabled`,
        );
      }
    }
  }
...
for (const path of examples) {
  ...
  if (
    /-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----|\bsb_secret_[A-Za-z0-9._-]{16,}/.test(
      content,
    )
  ) {
  ...
  if (/\bWORKER_(?:OLD|NEW)_IMAGE=/.test(content)) {
    validateDrainEnvironment(
      environment,
      path,
      true,
      /drain\.handoff\.env\.example$/i.test(path),
    );
  } else if (/\b(?:WEB|API|WORKER)_IMAGE=/.test(content))
    validateApplicationImages(environment, path, true);
}

const processImages = Object.fromEntries(
  ["WEB_IMAGE", "API_IMAGE", "WORKER_IMAGE"].flatMap((key) =>
    process.env[key] ? [[key, process.env[key]]] : [],
  ),
);
if (Object.keys(processImages).length > 0)
  validateApplicationImages(processImages, "process environment", false);
...
for (const relativePath of releaseManifestArguments) {
  ...
  if (manifest?.schemaVersion !== 1)
    errors.push(`${path}: unsupported release manifest schemaVersion`);
  const source = manifest?.source;
  const sourceKeys =
    source && typeof source === "object" ? Object.keys(source).sort() : [];
  const expectedSourceKeys = ["commit", "event", "ref", "repository", "runId"];
  if (JSON.stringify(sourceKeys) !== JSON.stringify(expectedSourceKeys))
    errors.push(
      `${path}: source must contain exactly repository, commit, ref, event, and runId`,
    );
  if (!source || !/^[a-f0-9]{40}$/.test(source.commit ?? ""))
    errors.push(`${path}: source.commit must be a full Git SHA`);
  if (source?.repository !== "ListenCodes/infinite-canvas")
    errors.push(`${path}: unexpected source repository`);
  if (!Number.isSafeInteger(source?.runId))
    errors.push(`${path}: invalid workflow run identity`);
  const tagPush =
    source?.event === "push" && /^refs\/tags\//.test(source?.ref ?? "");
  ...
  if (JSON.stringify(actualServices) !== JSON.stringify(expectedServices))
    errors.push(`${path}: images must contain exactly api, web, and worker`);
  for (const service of expectedServices) {
    ...
    if (!/^sha256:[a-f0-9]{64}$/.test(image.digest ?? ""))
      errors.push(`${path}: invalid ${service} digest`);
    if (image.reference !== `${repositoryName}@${image.digest}`)
      errors.push(`${path}: ${service} reference does not match its digest`);
    ...
    if (
      ...
      errors.push(
        `${path}: ${service} must contain exactly linux/amd64 and linux/arm64`,
      );
    if (
      platforms.some(
        (platform) => !/^sha256:[a-f0-9]{64}$/.test(platform.digest ?? ""),
      )
    )
      errors.push(`${path}: ${service} has an invalid child digest`);
    const expectedTags = [];
    if (tagPush) {
    ...
    if (
      ...
      JSON.stringify(expectedTags.sort())
    )
      errors.push(
        `${path}: ${service} tags do not match source/release identity`,
      );
    for (const environment of validatedEnvironments) {
      if (environment[`${service.toUpperCase()}_IMAGE`] !== image.reference)
        errors.push(
          `${path}: ${service} reference differs from release-images.env`,
        );
    }
  }
  if (tagPush !== (typeof manifest.releaseTag === "string"))
    errors.push(`${path}: releaseTag must exist only for a tag push`);
  if (tagPush && source.ref !== `refs/tags/${manifest.releaseTag}`)
    errors.push(`${path}: releaseTag does not match source.ref`);
}
