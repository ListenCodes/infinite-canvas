DROP FUNCTION app.claim_generation_attempt(uuid, uuid, uuid, uuid, uuid, uuid, generation_capability, text);
...
CREATE FUNCTION app.claim_generation_attempt(
  ...
  p_executor_dispatch_token uuid,
...
BEGIN
  ...
  SELECT attempt.status, attempt.executor_dispatch_token, attempt.executor_claim_id, job.current_attempt_id
  INTO current_status, current_dispatch_token, current_claim, current_attempt_id
  ...
  IF NOT FOUND OR current_attempt_id IS DISTINCT FROM p_attempt_id OR
    ...
    RETURN 'terminal';
  END IF;
  IF current_dispatch_token IS DISTINCT FROM p_executor_dispatch_token THEN
    RETURN 'duplicate';
  END IF;
  ...
  IF current_status = 'created' THEN
    ...
    WHERE id = p_attempt_id AND status = 'created'
      AND executor_dispatch_token = p_executor_dispatch_token
      AND executor_claim_id IS NULL;
  ELSIF current_status IN ('claimed', 'submitting', 'accepted', 'materializing') THEN
    ...
    WHERE id = p_attempt_id AND status = current_status
      AND executor_dispatch_token = p_executor_dispatch_token
      AND executor_claim_id IS NULL;
  ELSE
    RETURN 'duplicate';
...
REVOKE ALL ON FUNCTION app.claim_generation_attempt(uuid, uuid, uuid, uuid, uuid, uuid, generation_capability, uuid, text) FROM PUBLIC;
...
BEGIN
  ...
  LOOP
    EXECUTE format(
      'GRANT EXECUTE ON FUNCTION app.claim_generation_attempt(uuid, uuid, uuid, uuid, uuid, uuid, generation_capability, uuid, text) TO %I',
      runtime_role
    );
  END LOOP;
END
