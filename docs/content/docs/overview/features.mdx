...
- Persist projects in the browser in local mode, or explicitly publish project snapshots to the service in Hatchet cloud mode.
...
- In local mode, connect to user-provided OpenAI-compatible endpoints directly from the browser.
- In cloud mode, create durable image and video tasks through the Fastify API and Hatchet Worker without exposing platform credentials to the browser.
- Model a three-image request as one batch with three independent jobs, retaining successful slots and retrying only failed slots.
- Resume tasks after closing or refreshing the browser through durable snapshots and SSE events; uncertain paid submissions enter outcome reconciliation instead of being blindly retried.
...
- Keep local-mode application data and user credentials in the browser.
- Use WebDAV for optional cross-device synchronization.
- When the cloud runtime is configured, sign in with Supabase Auth, publish projects without deleting the local source, and manage durable tasks and server-side assets.

## Current limitations

- A Web-only deployment remains local mode; projects, assets, and user API keys stay in the browser.
- Cloud mode requires the API, Worker, PostgreSQL, Supabase Auth, private object storage, and Hatchet services described in the operations documentation.
- Browser-side custom invocation scripts remain a local-mode feature and are never executed by the Worker.
